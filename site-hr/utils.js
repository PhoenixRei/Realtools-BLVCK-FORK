...
function initializeHorseProfile(doc = undefined) {
    ...
    const modernHost = d.querySelector?.('horsereality-horse')
    const modernRoot = modernHost?.shadowRoot
    const collected = {
        name: d.title.replace(/ - Horse Reality$/, ''),
        tagline: null,
        sex: d.querySelector('img.icon16')?.alt || modernRoot?.querySelector('#sex')?.innerText.trim() || '',
        adult_layer_keys: [],
        foal_layer_keys: [],
        looking_at: null,
    ...
    if (modernRoot) {
        collected.name = modernRoot.querySelector('#name')?.innerText.trim() || collected.name
        collected.breed = modernRoot.querySelector('#breed')?.innerText.trim() || collected.breed
        collected.lifenumber = Number(modernHost.getAttribute('horseid')) || collected.lifenumber
        const age = modernRoot.querySelector('#age')?.innerText.trim() || ''
        collected.looking_at_foal = /^\d+\s+months?$/i.test(age) && Number.parseInt(age, 10) < 12
        const modernLayers = modernRoot.querySelectorAll('#profile-image hr-horse-image-layer[url]')
        for (const layer of modernLayers) {
            const url = layer.getAttribute('url')
            if (!url || url.includes('/images/profilebg_')) continue
            const key = new URL(url).pathname.split('/').pop().replace(/\.png$/i, '')
            if (collected.looking_at_foal) collected.foal_layer_keys.push(key)
            else collected.adult_layer_keys.push(key)
        }
    }
